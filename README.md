...
This package makes it easy to send notifications using [SNS](link to service) with Laravel 5.3.

**Note:** Replace ```AwsSns``` ```SNS``` ```Menaf Gul``` ```:author_username``` ```www.menafgul.com``` ```hi@menafgul.com``` ```aws-sns``` ```:package_description``` ```:style_ci_id``` ```:sensio_labs_id``` with their correct values in [README.md](README.md), [CHANGELOG.md](CHANGELOG.md), [CONTRIBUTING.md](CONTRIBUTING.md), [LICENSE.md](LICENSE.md), [composer.json](composer.json) and other files, then delete this line.
